...
export default function RoleSelectModal({ onClose }) {
  ...
  const roles = [
    ...
    { icon: '⭐', label: 'All Rounder', price: '₹2,000', color: '#C9A227', bg: '#FFF8E1', formUrl: settings.allRounderFormLink || 'https://forms.gle/thqKgAwsuTqt4VARA', popular: true },
  ]
  ...
  useEffect(() => {
    const h = (e) => { if (e.key === 'Escape' && !selectedRole) onClose() }
    ...
    <>
          ...
          >
            ...
            <div className="px-6 py-4 flex items-center justify-between border-b border-gray-100">
              ...
              <button onClick={onClose} aria-label="Close"
                className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-500 text-sm transition-colors">✕</button>
            </div>
            <div className="px-6 pt-5 flex flex-col gap-3">
              {roles.map((role, i) => (
                ...
                >
                  {role.popular && (
                    <span className="absolute -top-2.5 right-4 text-white text-[10px] font-black px-3 py-0.5 rounded-full uppercase tracking-widest"
                      style={{ backgroundColor: role.color }}>Popular</span>
                  )}
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                    style={{ backgroundColor: role.color + '20' }}>{role.icon}</div>
                  <div className="flex-1">
                    <p className="font-black text-gray-800 text-base">{role.label}</p>
                    <p className="text-xs mt-0.5 font-semibold" style={{ color: role.color }}>{role.price} registration fee</p>
                  </div>
                  <svg className="w-5 h-5 flex-shrink-0" style={{ color: role.color }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </motion.button>
              ))}
              <p className="text-center text-xs text-gray-400 pt-1">Select your role to view terms and proceed</p>
            </div>
          </motion.div>
        </motion.div>
      </AnimatePresence>
      ...
      {selectedRole && (
        <TermsModal
          ...
          onClose={() => { setSelectedRole(null); onClose() }}
        />
      )}
    </>
  )
